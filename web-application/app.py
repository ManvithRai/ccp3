...
def download_file(filename):
	...
	if(os.path.isfile(filepath)):
		return send_file(filepath, attachment_filename='download.txt',as_attachment=True)
	else:
		return render_template('file-list.html',msg='An issue encountered')
...
def download_f():
	for root,dirs,files in os.walk(UPLOAD_FOLDER):
		if(len(files) == 0):
			return render_template('file-list.html',msg='No file found in directory')
		else:
